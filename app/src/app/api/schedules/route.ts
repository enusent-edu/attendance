...
export async function POST(req: Request) {
  ...
  // schedules: [{ member_no, date, shift_start, shift_end, department }]
  // Pre-load all groups for this org once (name → id lookup)
  const { rows: allGroups } = await db.query(
    `SELECT id, name FROM attendance.groups WHERE org_id=$1`, [s.org_id]
  )
  const groupByName: Record<string, string> = {}
  allGroups.forEach(g => { groupByName[g.name.toLowerCase().trim()] = g.id })
  ...
  for (const row of schedules) {
    try {
      ...
      const { rows: mRows } = await db.query(
        ...
        [String(row.member_no).trim(), s.org_id]
      )
      if (!mRows.length) { errors.push(`Member not found: ${row.member_no}`); continue }

      // Resolve department name → group_id
      const deptKey = String(row.department || row.group_id || '').toLowerCase().trim()
      const group_id = groupByName[deptKey]
      if (!group_id) { errors.push(`Department not found: "${row.department || row.group_id}" for member ${row.member_no}`); continue }

      await db.query(
        `INSERT INTO attendance.member_schedules (org_id,member_id,group_id,date,shift_start,shift_end,created_by)
         VALUES ($1,$2,$3,$4,$5,$6,$7)
         ON CONFLICT (member_id,date) DO UPDATE SET shift_start=EXCLUDED.shift_start, shift_end=EXCLUDED.shift_end, group_id=EXCLUDED.group_id`,
        [s.org_id, mRows[0].id, group_id, row.date, row.shift_start, row.shift_end, s.id]
      )
      inserted++
    } catch (e: unknown) {
